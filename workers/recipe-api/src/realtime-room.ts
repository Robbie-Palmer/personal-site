...
function parseChangeEvent(value: unknown): PantryRealtimeEvent | undefined {
  ...
  if (
    ...
    !event.pantry ||
    typeof event.pantry !== "object" ||
    event.pantry.resourceId !== event.resourceId ||
    event.pantry.revision !== event.revision ||
    event.pantry.operationId !== event.operationId ||
    !event.pantry.stock ||
    typeof event.pantry.stock !== "object" ||
    !event.pantry.itemVersions ||
    typeof event.pantry.itemVersions !== "object"
  ) {
    return undefined;
  }
