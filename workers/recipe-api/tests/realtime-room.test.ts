...
function changeEvent(
  ...
  return {
    ...
    pantry: {
      ...
      stock: changeKind === "pantry.item-removed" ? {} : { onion: "fresh" },
      itemVersions:
        changeKind === "pantry.item-removed" ? {} : { onion: revision },
    },
  } as const;
}
...
describe("HouseholdRealtimeRoom", () => {
  it("fans a validated revision invalidation out to authorized sockets", async () => {
    ...
    const event = changeEvent("42", "pantry.item-set");
  ...
  it("disconnects unauthorized and failed recipients during publication", async () => {
    ...
    const response = await room.fetch(
      new Request("https://room.test/publish", {
        method: "POST",
        body: JSON.stringify({
          event: changeEvent("43", "pantry.item-removed"),
          authorizedUserIds: ["user-1"],
        }),
      }),
    );
  ...
  it("revokes removed household members before publishing", async () => {
    ...
    const event = changeEvent("44", "pantry.item-set");
