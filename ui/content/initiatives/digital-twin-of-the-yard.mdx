...
Extraction -->|uncertain| InLoop["Human-in-the-loop<br/>verify or correct"]:::person
...
Operations --> Yard
Operations --> OnLoop["Human-on-the-loop<br/>monitor or override"]:::person
...
People contribute to that history rather than sitting outside the model. A gate guard or dispatcher can verify an uncertain extraction or correct a mismatched record before the workflow continues. For automated decisions, a supervisor can monitor outcomes, override an action, or change the operating policy. Those decisions return to the shared event stream with their source and time, so the twin records what people knew, changed, and approved alongside system observations.
