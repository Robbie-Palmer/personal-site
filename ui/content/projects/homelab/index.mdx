...
* **Old, unsupported 1050 GPU.** The box turned out to have a GTX 1050,
  Pascal-era (`sm_61`), not the 1060 the ADR first assumed. Same problem
  either way: CUDA 13 and the newer driver branches dropped Pascal, so the
...
* **Reachable over the tailnet.** The desktop has no ethernet run to the
  router, so its uplink is an Android phone over USB tethering. That link
