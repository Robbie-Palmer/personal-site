...
The hub's harness lineup is Claude Code, Codex, and Grok Build, three
proprietary harnesses, each tied to a commercial model family and subscription.
I want at least one open-source harness in the mix for the same reasons the
...
OpenCode is an open-source, terminal-based coding harness that is intentionally
...
third-party providers anyway. Open weights create provider competition without
putting a model-serving box in my house.

# Decision

Install **OpenCode** on the Mac mini alongside the proprietary harnesses.
...
## Open-source harnesses other than OpenCode (Aider, Continue)
...
## No open-source harness; rely on the three commercial ones
...
* **Complements the commercial harnesses** as the fourth in the lineup, all
...
* **Not the primary harness**: by design it's the fallback, so it gets less
