...
A Kubernetes Secret manifest only base64-encodes its data; it does not encrypt
it. Configure API-server encryption at rest for Secret objects separately. Do
not commit plain credentials or generated Secret manifests.

[ADR 024](/projects/homelab/adrs/024-doppler-secrets) makes Doppler the source
of truth. The pilot may create its first Secret through a local mise command
that does not print values. Before unattended workloads need rotation, install
the Doppler Kubernetes Operator with a read-only service token scoped to the
homelab config. Do not put a personal Doppler CLI token in the cluster.
...
the fleet. During the pilot, keep Compose only as the rollback path. K3s remains
the target workload API.
