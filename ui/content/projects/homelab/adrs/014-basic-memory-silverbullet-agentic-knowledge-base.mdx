...
* Make the live space a private Git repository on the Mac mini's internal
  ...
  easy to undo. The separate backup system still owns recovery.
...
3. Before a broad graph-driven mutation, the agent lets file watchers settle
   and reads each target file from disk. Use the index only for discovery.
   It then checkpoints the Git worktree, makes small inspectable
...
* **Cons**: Direct file tools provide no consistent, graph-aware contract across
  the four harnesses. Coupling the agent contract to editor extensions would also
...
* **Pros**: The strongest polished-client and extension option over the same
  ...
  budget ceiling.
* **Cons**: Obsidian's core is proprietary. Sync is a hosted service with no
  self-hosted server, and Headless Sync is still in open beta. Community
  plugins run with broad application permissions and vary in maintenance
  quality. It has no web client, and ecosystem enthusiasm can itself become
...
* **Cons**: It primarily supports notebooks and tags, with less flexible
  graph modelling. Joplin warns that its filesystem sync format is
