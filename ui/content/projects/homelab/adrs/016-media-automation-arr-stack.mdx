...
supply. Every episode I wanted meant finding a torrent by hand, waiting for
it in a GUI client, renaming the file to something Jellyfin parses, moving it
into the right season folder, and rescanning the library. Silo season three
went like that, file by file, and each step was toil I will repeat every week
for every show.
...
Run the standard four-piece acquisition stack on the Mac mini, in one Compose
...
* **qBittorrent** (containerized) downloads torrents into
  ...
  for why this replaces the desktop clients.
...
```text
Prowlarr ──sync──▶ Sonarr ──grab──▶ qBittorrent ──import──▶ /media/TV ──▶ Jellyfin
   │                ▲                    │
   │             Radarr                 │
   └──sync──────────┴──────◀─────────────┘
        (searches indexers, sends magnets)
```

Wiring between the four services happens through their REST APIs, driven by
an idempotent `provision.py` script rather than hand-clicking in four web
...
* Four more long-running services on a 4GB VM alongside Jellyfin,
  SilverBullet, and the photo backup. Memory pressure is the first thing to
  watch.
* Public-tracker reachability from a .NET HTTP stack is fragile: eztv's CDN
  rejects Prowlarr's TLS handshake outright, so that indexer stays disabled.
