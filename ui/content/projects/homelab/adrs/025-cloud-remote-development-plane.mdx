...
durable state 45 GiB, its rebuildable tool and package caches 30 GiB, and the
pilot workspace 10 GiB. Leave the remaining filesystem capacity unallocated
so ext4 metadata and quota enforcement have headroom. Kubernetes local volumes
describe these paths, while ext4 project quotas enforce their limits on the
single physical filesystem.
