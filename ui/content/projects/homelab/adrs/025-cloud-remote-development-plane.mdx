...
Provision one long-lived Hetzner Cloud VPS as a separate, single-user
remote-development plane. Terraform owns the Hetzner server, firewall, SSH key
registration, and persistent volume. A NixOS flake owns the operating system,
Tailscale, storage mounts, and a single-node K3s server. K3s owns t3-code and
its supporting workloads.
...
Give the host a zstd-compressed zram swap device capped at 25 percent of
physical RAM. Configure kubelet to tolerate host swap while keeping `NoSwap`
behavior for pods. The buffer protects host services during short memory
spikes without allowing workloads to exceed their declared memory limits.
...
Keep the K3s datastore and container image store on the VPS root disk. Keep
workspace state on the attached encrypted volume. A workspace filling its
volume must not also exhaust K3s image storage, taint the node with disk
pressure, and remove the image needed to restart that workspace.

Split the attached volume into project-quota domains. Give the operator's
durable state 55 GiB and its rebuildable tool and package caches 30 GiB. Leave
the remaining filesystem capacity unallocated so ext4 metadata and quota
enforcement have headroom. Kubernetes local volumes describe these paths,
while ext4 project quotas enforce their limits on the single physical
filesystem.

Keep active worktrees, t3-code state, repositories, and agent authentication
in the durable volume. Put mise installations, the pnpm store, browser caches,
and Arduino packages in the cache volume. Cache loss may cost download time,
but it must not lose a branch, thread, or login.

Use disk-backed `emptyDir` volumes for process scratch space. Set an 8 GiB
limit for the operator's `/tmp` and declare pod ephemeral-storage requests and
limits. Do not use memory-backed temporary storage on this 8 GiB host.

Delete a worktree through t3-code's thread lifecycle. Automated housekeeping
may remove a worktree only after its thread is deleted, or report a clean,
merged, idle thread for bulk deletion. It must not remove a directory owned by
a running thread directly from the host.
...
* The VPS is intentionally single-user. A second person requires separate
  workspace compute and a recovery design proved outside this host.
...
* Tool and package caches now have a hard ceiling. A full cache can make an
  install fail until housekeeping prunes it, but it cannot consume workspace
  or K3s capacity.
* Local persistent volumes and ext4 quotas remain specific to this one-node
  design. A multi-node workspace service would need networked or replicated
  storage instead.
