...
* **Integrates with Mise**: Build on top of the task system established in [ADR 004: Mise](/projects/personal-knowledge-graph/adrs/004-mise), not duplicate configuration
...
```json
{
  "lint-staged": {
    ...
    "infra/public-platform/**/*.tf": ["mise run //infra/public-platform:format", "mise run //infra/public-platform:precommit-lint"],
    "infra/bootstrap/**/*.tf": ["mise run //infra/bootstrap:format", "mise run //infra/bootstrap:precommit-lint"],
...
* **Monorepo Awareness**: Supports workspace-specific tasks (`//ui:format`, `//infra/public-platform:format`) while maintaining repo-wide consistency for shared file types (Markdown, YAML).
...
* **Claude Code Guardrails**: Prevents [Claude Code](/projects/personal-knowledge-graph/adrs/012-claude-code) from drifting too far and committing malformed code. Pre-commit hooks act as an immediate correction signal, keeping the AI agent aligned with project standards without requiring constant human oversight.
