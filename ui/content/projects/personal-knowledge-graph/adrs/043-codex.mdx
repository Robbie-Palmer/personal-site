...
[ADR 012: Claude Code](/projects/personal-knowledge-graph/adrs/012-claude-code) records why this project uses an agentic coding assistant: deep reasoning, repository-wide changes, and autonomous iteration are central to maintaining velocity as a solo developer. This ADR accepts that basis without repeating it.
...
I also experienced severe Claude availability problems earlier in 2026. I have repeatedly perceived both availability and output quality declining around major model releases. Whether releases cause the degradation remains unproven. The operational effect is the same: relying on Claude alone can reduce velocity when demand is highest.
...
**[Claude Code on the web](https://code.claude.com/docs/en/claude-code-on-the-web)** supports mobile development in isolated Anthropic-managed environments. This provides a useful security boundary and works while a personal computer is unavailable. Fresh environments can require dependencies, tools, and project context to be prepared again.
...
Codex joins Claude Code as a second coding assistant. Using both tools preserves access to their different models, interfaces, and workflows while reducing dependence on one provider's availability, pricing, and usage policy.
...
* **Narrower Mobile Feature Set**: Codex's mobile remote workflow requires the Mac mini, while Claude Code supports both hosted cloud environments and connected-host sessions. Added capacity and resilience justify this adoption despite that limitation.
