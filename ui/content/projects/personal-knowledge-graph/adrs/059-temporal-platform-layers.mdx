...
[ADR 015](/projects/personal-knowledge-graph/adrs/015-ssg),
[ADR 033](/projects/personal-knowledge-graph/adrs/033-content-graph-indexes), and
[ADR 038](/projects/personal-knowledge-graph/adrs/038-content-graph).
...
Projects combine these layers independently and explicitly adopt every layer
that applies. Layers do not extend one another: sharing one layer never
silently opts a project into another.
...
Pydantic and pytest-cases. The infrastructure layer may prefer Terraform for
...
* `database.relational-host`, which may select Neon when PostgreSQL needs a
  managed host;
* `database.olap-engine`, which may select DuckDB.
...
transactional storage and analytical processing. Choose the broadest scope for
which the platform can defend one default against real alternatives under
substantially shared forces.

The base layer has a required `project.primary-language` slot whose initial
selection is TypeScript. The TypeScript layer declares that a resolved
TypeScript choice activates it. A project that needs Python's machine-learning
ecosystem records a local override of `project.primary-language`; the Python
layer declares the corresponding activation condition. A project may still
declare supplemental languages directly without replacing its primary
language.
...
selections:
  - technology: "cloudflare-workers"
    ...
    effective_from: "2025-11-12T00:00:00Z"
    decision: "personal-engineering-platform:002-cloudflare-workers"
    origin_projects: ["personal-knowledge-graph"]
```
...
period. Resolution includes the layer-slot policies, technology selections,
project layer uses, and project slot uses effective at the query timestamp.
The final stack for a completed or frozen project uses its closure timestamp.
Later platform decisions do not alter it.
...
therefore resolves PostgreSQL and Neon while leaving DuckDB as an unused
recommendation.
...
Keep platform layers and the temporal policy, selection, and usage records in
the content model so queries retain their dates and provenance. Present the
reader-facing knowledge graph through direct relationships:

```text
Platform OWNS_LAYER Layer
Project USES_PLATFORM_LAYER Layer
Layer REQUIRES_TECHNOLOGY Technology
Layer PREFERS_TECHNOLOGY Technology
ADR DRIVEN_BY Project
```

Layer-slot policies, default selections, layer uses, and slot uses keep their
identity in the query model because they carry status or time. They are edge
provenance in the serialized visualization, not graph nodes. This preserves
the evidence without forcing readers to traverse implementation vocabulary
such as slot policy, default selection, and layer use.
...
relation retains its source layer, slot policy, selection, and deciding ADR.
Direct project technologies and platform-provided technologies remain
distinguishable in queries and views.
...
* Slot policies, layer uses, and default selections become association records
  with their own schemas, validation, indexes, and queries.
...
The representative migration used these acceptance checks:

1. Schemas validate platform layers, stable slots, dated layer-slot policies,
   slot prerequisites, default selections, selection status, supersession,
   language-layer activation, and dated project layer and slot uses. They
   reject date-only temporal values.
...
3. Historical queries resolve slot policies, selections, and project uses at
   the requested timestamp. They keep completed projects fixed at closure and
   do not link pre-platform or organisation-governed projects implicitly.
...
6. Database resolution can select PostgreSQL with Neon hosting or DuckDB for
   OLAP without implying that every database-backed project uses all three
   technologies. Observability resolution likewise distinguishes application
   telemetry from node monitoring.
...
10. Project pages show project-specific technology badges separately from a
    quiet `Built on` metadata line and secondary platform-provided
    technologies.
