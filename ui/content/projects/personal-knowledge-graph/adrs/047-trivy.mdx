---
title: "ADR 047: Trivy Security Scanner"
date: "2026-06-21"
status: "Accepted"
tech_stack: ["Trivy"]
---
...
* **CodeQL**: SAST for source code vulnerabilities (SQL injection, XSS, etc.) ([ADR 032](/projects/personal-knowledge-graph/adrs/032-codeql))
* **Renovate**: Automated dependency updates with CVE visibility and security issue prioritization ([ADR 024](/projects/personal-knowledge-graph/adrs/024-renovate))
...
Results are uploaded to GitHub Security tab in SARIF format for unified visibility alongside CodeQL findings. PR gating uses GitHub's Code Scanning alert checks (the same mechanism as CodeQL ([ADR 032](/projects/personal-knowledge-graph/adrs/032-codeql))), which distinguish new alerts from pre-existing ones. Only vulnerabilities introduced by a PR block it.
...
* **Gates New Dependencies**: PRs that introduce dependencies with known CRITICAL or HIGH CVEs are blocked before merge. Complements Renovate ([ADR 024](/projects/personal-knowledge-graph/adrs/024-renovate)) which handles updates to existing dependencies.
...
* **New vs Existing Distinction**: SARIF upload to GitHub Code Scanning blocks a PR only for issues it introduces. Pre-existing vulnerabilities remain visible without turning the scanner into a blanket blocker.
