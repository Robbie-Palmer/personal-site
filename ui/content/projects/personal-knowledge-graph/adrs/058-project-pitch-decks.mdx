...
deep links, speaker notes, fullscreen presentation, and scroll view. The same source will also produce a
...
focus-scoped keyboard shortcuts, touch navigation, URL state, and media autoplay control.

### Keep overview and scroll views under React ownership

Use reveal.js for the live slide presentation, navigation, fragments, hashes, fullscreen, and speaker notes. Render
the overview and linear scroll modes as local React views from the same slide children. Disable reveal.js's native
overview and automatic scroll activation.

This boundary is necessary because reveal.js 6's scroll controller reparents each slide into generated page nodes,
stores the presentation as HTML, and restores it with `innerHTML`. The React wrapper still treats the original slide
tree as React-owned. A later React render can therefore reconcile against a DOM structure that reveal.js has moved.
The reveal.js visibility update also reapplies `hidden` to non-current slides after navigation, which conflicts with
an overview that must keep every slide visible. Keeping the two multi-slide views in React avoids both ownership
conflicts while retaining reveal.js where it is strongest.
...
fullscreen, and speaker views on wide screens. The project-page embed will keep hash navigation off so slide state
cannot collide with heading anchors in the overview.
...
### Keep mobile in slide mode

On wide screens, the embedded deck will use a 16:9 stage and may offer the React-owned linear scroll view. On narrow
screens, use a 3:4 stage with reveal.js's horizontal touch navigation. Do not expose or automatically activate the
scroll view on mobile. This keeps the project-page embed, focused route, and the narrow receiver iframes used by
speaker view on the same presentation state.

The overview becomes a two-row horizontal track on narrow screens so touch users can pan through its thumbnails.
The transcript remains the linear reading alternative.
...
### Keep version one on the web

Version one will support the web deck and its plain-text transcript. It will not expose reveal.js's print mode or
a PDF action. Browser printing adds another layout lifecycle to an already interactive route without a current
download or archival need. If that need appears later, a build task should create and verify a reproducible PDF
rather than handing raw browser print behavior to readers.
...
editorial work of rewriting every project. They are planning estimates based on repository inspection. Vendor
claims do not determine them.
...
site. Use it as a reference without adding it as a dependency.
...
5. Test static export, React 19 lifecycle, bundle loading, keyboard focus, speaker notes, wide-screen scroll view,
   and mobile touch navigation.
...
5. Wide-screen slide view and narrow-screen swipe view remain readable without clipped content.
...
7. The focused route supports a direct slide URL, fullscreen, speaker notes, and overview mode.
...
10. Unit tests cover deck discovery, frontmatter validation, slide grouping, transcript conversion, and tabs. A
    ...
    navigation, focus-scoped keys, the focused route, mobile swiping, and touch panning in the overview.
...
* Focused, embedded, scroll, and presenter views come from one source.
