---
title: "ADR 050: Simple Icons and SVGL"
date: "2026-06-30"
status: "Accepted"
...
* Simple Icons deliberately normalizes logos into compact 24×24-style marks, which is not always the best fit for a portfolio page.
...
* **Pros**: Broad brand coverage, first-party npm package, CDN support, TypeScript-friendly data, consistent viewboxes, hex colors, source URLs, and a public contribution workflow. The Simple Icons repository is CC0, and the package includes metadata for individual icon sources, licenses, and brand guidelines when available.
...
* **Decision**: Rejected as the only source. Use it only when a Simple Icons entry is missing or unsuitable.
...
* **Cons**: It wraps icon delivery in React without addressing logo sourcing. It does not solve missing SVGL logos, full-color SVGs, wordmarks, or local provenance. It also couples content assets to the React bundle rather than serving them as static content.
