...
* **`terraform fmt` + `terraform validate`** ([ADR 010](/projects/personal-knowledge-graph/adrs/010-terraform))
...
* **Trivy's IaC config scan** ([ADR 047](/projects/personal-knowledge-graph/adrs/047-trivy))
...
[ADR 048 (SonarQube)](/projects/personal-knowledge-graph/adrs/048-sonarqube), the way
[ADR 049 (zizmor)](/projects/personal-knowledge-graph/adrs/049-zizmor) did for workflows:
...
[mise](/projects/personal-knowledge-graph/adrs/004-mise) so CI and laptop run the same
pinned version:
...
* `mise run //infra/public-platform:lint:tflint` / `//infra/bootstrap:lint:tflint` tasks, wired
...
* A dedicated **TFLint CI** workflow, triggered by any `*.tf` or `.tflint.hcl`
  ...
  `infra/bootstrap/`-only changes don't trigger a no-op plan of `infra/public-platform/` behind
...
Weighted by the same test as [ADR 047](/projects/personal-knowledge-graph/adrs/047-trivy)
and [ADR 049](/projects/personal-knowledge-graph/adrs/049-zizmor): deterministic, free,
...
* **Rule preset drift.** `recommended` evolves across TFLint versions; a mise
  ...
  rather than a silent change ([ADR 024](/projects/personal-knowledge-graph/adrs/024-renovate)).
