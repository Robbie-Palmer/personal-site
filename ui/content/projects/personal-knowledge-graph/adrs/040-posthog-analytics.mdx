...
Cloudflare provides basic analytics in its dashboard (page views, unique visitors, top paths), but the data is extremely limited and the UX for exploring it is poor. It offers no way to track user flows, define success metrics, run experiments, or build a meaningful feedback loop from Cloudflare's built-in analytics alone.
...
* **Additional JavaScript**: The `posthog-js` SDK adds client-side JavaScript, increasing bundle size and potentially impacting page load performance. Mitigated by PostHog's lightweight SDK and the site's static generation strategy [ADR 015](/projects/personal-knowledge-graph/adrs/015-ssg).
