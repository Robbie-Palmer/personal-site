...
secrets ([ADR 035](/projects/personal-knowledge-graph/adrs/035-doppler)), GitHub
environments hold CI credentials
([ADR 018](/projects/personal-knowledge-graph/adrs/018-github-secrets)). The unsolved
...
1. **It fires after the push.** On a public repo that is too late by
   ...
   rotation. Post-push detection reports an incident after the guard has
   already failed.
...
On the determinism axis of [ADR 048](/projects/personal-knowledge-graph/adrs/048-sonarqube):
...
* **Pre-commit** (the layer that actually prevents leaks): a
  ...
  terminal scrollback.
* **CI, Security-tab posture** ([ADR 047](/projects/personal-knowledge-graph/adrs/047-trivy) /
  [ADR 049](/projects/personal-knowledge-graph/adrs/049-zizmor) pattern): a workflow scans
  ...
  and historical findings require rotation. A red build cannot remediate them.
...
for years, with a stable CLI, huge deployment base, deep LLM corpus
...
* **Pattern-based detection is not proof of absence**. A secret in a shape no
  rule anticipates still gets through; pre-commit reduces the risk without
