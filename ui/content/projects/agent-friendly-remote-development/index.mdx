...
Infrastructure as code is part of the product, not just an operator preference.
An agent should be able to inspect the declared infrastructure, propose a
change, rebuild a host, and remove a test environment without depending on
undocumented console state.

[ADR 003](/projects/agent-friendly-remote-development/adrs/003-hetzner-cloud-for-pilot-infrastructure)
keeps Hetzner Cloud for the pilot and the next host under the current
architecture. Its official Terraform provider covers the complete small-server
lifecycle needed here, and the NixOS deployment has already run real work.

The previous evaluation rejected OVHcloud because of weak infrastructure as
code support. Its official Terraform provider can now order and install a VPS,
but that part of the provider remains incomplete. Open upstream work describes
current VPS API coverage as about 15% and would add 20 resources and 40 data
sources. OVHcloud's lower list price and included backup do not outweigh those
lifecycle gaps when agent-operated, reproducible infrastructure is the leading
criterion. Hostinger's young provider and netcup's API-first path are weaker
still. Oracle has mature IaC, but reclaimable free ARM64 capacity and OCI's
account, network, and policy model do not fit this continuously available
single-node pilot.

kube-hetzner strengthens the Hetzner growth path. This Terraform module builds
K3s or RKE2 clusters; it does not sell hosting. Direct K3s on the declared NixOS
host stays smaller today; kube-hetzner becomes relevant for a new multi-node
cluster with automated node replacement or autoscaling.
...
# Managed runtime cost comparison
...
Code or a task prevents automatic sleep. This table compares the current host
with managed development runtimes. ADR 003 records the infrastructure-provider
decision. The figures below exclude model subscriptions, tax, egress, and
support.
...
egress add-ons, backups, and support. Other low-cost VPS plans can undercut it,
but the project requires a complete declarative lifecycle before comparing
price. Managed development providers become competitive only when they suspend
idle environments, but that needs a real T3 Code test. Fly counts open TCP
connections as activity, so a persistent connection could turn a sleep-priced
design into an $80 or higher monthly environment.
...
operate the open source deployment on their own low-cost VPS remain the
...
* Can an external resource gateway revoke one agent without exposing its
  upstream credential?
* Can the provider resources and NixOS host be rebuilt from versioned code in a
  clean project without an undocumented console step?
