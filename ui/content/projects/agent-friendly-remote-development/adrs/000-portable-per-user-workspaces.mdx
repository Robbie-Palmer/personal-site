...
The runtime provider may provide a credential connector or gateway, but it
must not become the source of truth for the user's grants.

Keep the current Hetzner deployment as the working baseline. Require the host
and provider resources to be reproducible from versioned infrastructure as
code; a routine web-console step fails the portability test. Test Coder
Community on the existing K3s cluster before deciding whether to build user and
workspace management directly:
...
7. Revoke the pilot user's workspace, Doppler, and repository access without
   affecting the operator's environment. Per-agent revocation remains outside
   this pilot.
...
# Why not another ordinary VPS provider

An ordinary VPS provider can run the same NixOS or container workload. It may
improve geography, support, price, or account preference, but it does not remove
the work of user identity, provisioning, suspension, metering, backup, and
offboarding. Provider selection follows the workspace contract rather than
defining it. An ordinary VM per user remains the simple hard-isolation fallback
