---
title: "ADR 010: Layered observability for agent workspaces"
date: "2026-09-12"
status: "Proposed"
tech_stack:
  - "Netdata"
...
The first deployment has one small host and one user. A future multi-user
cluster will use dedicated workspace nodes. A self-hosted
...
Run one pinned OpenTelemetry Collector Contrib service on the current host. T3
Code will send OTLP/HTTP to its private endpoint. A future multi-node cluster
will run an equivalent collector outside user namespaces. The
Collector will:
...
Use stable, non-personal workspace identifiers such as `operator` and an
opaque workspace ID.
...
and K3s container log rotation as fallbacks. Any later user must know what the
...
1. The Netdata dashboard works through its private URL without parent streaming
   or Netdata Cloud.
2. Netdata shows separate host and `operator` CPU, memory, pressure, throttling,
   PID, filesystem, byte-quota, and inode-quota views. The multi-node evaluation
   repeats this with a second opaque workspace ID.
...
11. Seven days of representative use produce no duplicate or unactionable
    pages. The sample records PostHog volume and projected cost, with a billing
    limit that prevents unplanned spend.
