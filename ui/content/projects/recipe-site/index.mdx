...
*Current implementation: Recipes are TypeScript objects with Zod schema validation, stored in git.
This gives us type safety, build-time error checking, and version history as architectural properties —
not user-facing features, but enablers for fast iteration. The recipe data format is an open decision
longer term — Cooklang's `.cook` markup and database-backed models are both under consideration
...
> The ML pipeline (`ml-pipelines/recipe-parsing/`) is being developed in parallel with Phases 2-3,
> not sequentially after them. Pipeline infrastructure (DVC stages, evaluation framework, ground-truth
> dataset) is already built. The phasing here reflects when ingestion features ship to users, not when
> development begins.

### Photo-to-Recipe

ML pipeline to extract structured recipe data from images — handwritten recipe pages, printed
cookbook photos, recipe cards. Enables batch digitization of existing physical recipe collections
and one-tap capture of any recipe image.
...
posts. This addresses the core pain point of recipes scattered across platforms and is the headline
feature of the AI-capture segment (Flavorish, Honeydew, Recify). Likely requires LLM-based
video/image analysis — distinct from both photo OCR and schema.org parsing.

### PR-Based Workflow

Ingested recipes (from photos, URLs, or social media) create pull requests for review before
merging into the recipe collection. Maintains quality control and the git-backed audit trail.
Longer term, the ingestion pipeline likely moves to writing directly to a database.

### Open Standard Data

Adopt [schema.org Recipe](https://schema.org/Recipe) as the canonical output format alongside the
internal TypeScript schema. schema.org is already the import source for website parsing — emitting
it for our own recipes enables SEO (search engines consume it), interoperability (other tools can
import our recipes), and export without vendor lock-in. Mealie already uses schema.org Recipe as its
core data model, confirming it's the lingua franca of the space.
...
original and its derivatives. Mealie's
[multi-generational collaboration model](https://cooklang.org/blog/09-cooklang-vs-paprika-vs-mealie/)
(a family in Italy using it to preserve a 100-year-old recipe collection across three generations
who contribute, comment, and adapt recipes) is a direct reference for how this should work.
...
[Treefrog Accelerator](https://treefrogaccelerator.com/) — validating investor interest in the recipe app
...
| **AI photo import**    | No            | No           | No        | AI        | AI                    | AI       | **Pipeline built** |
...
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **AI recipe import**           | [Flavorish](https://www.flavorish.ai/), [Honeydew](https://honeydewcook.com/), [Recify](https://www.recify.app/)                | Pipeline scaffolded, model in development                                                                                                                   |
| **Voice / hands-free cooking** | [Pestle](https://9to5mac.com/2022/01/21/pestle-cooking-app-for-ios-release/) (voice + wink), SideChef (voice + narrator)        | Future (post-cooking-mode, via Web Speech API)                                                                                                              |
...
content while adding dynamic capabilities only where needed. The recipe data format itself is
an open decision — the current TypeScript + Zod approach works well for the static site, but
Cooklang's `.cook` markup (with its WASM parser and inline ingredient/timer annotations) and
database-backed models are both worth evaluating as the project matures.

## Recipe Ingestion

Short/medium term: an ingestion pipeline (photo parsing, website import, social media extraction)
that creates PRs for review. Longer term: likely moves to writing directly to a database as the
content management needs outgrow git-based workflows.
