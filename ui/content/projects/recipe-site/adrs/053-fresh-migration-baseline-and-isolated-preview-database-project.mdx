...
Create a separate Neon project for preview databases. Its default root branch
is named `preview-base` and contains an empty `recipes` database plus the
preview-only `recipes_owner` role, but no application tables, production rows,
or Drizzle migration history. The GitHub preview environment receives a
project-scoped API key for this project only.
