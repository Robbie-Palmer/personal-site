---
title: "ADR 060: Agent Auth for Delegated Recipe Access"
date: "2026-07-25"
status: "Proposed"
...
* one or more item changes with stable item IDs, before/after values, and row versions
...
Undo must be concurrency-aware. It applies automatically when the current row version still matches
the original change's `after` version. If a person or another agent has edited the item since, the UI
shows a three-way preview and asks which values to restore. A batch reconciliation can be undone as
one unit or item by item.
