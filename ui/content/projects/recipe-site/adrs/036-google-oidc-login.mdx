...
their existing account; a new user gets one created on first sign-in. No separate
...
**The deployment will front the auth Worker on the site's own origin** rather than expose it on a
...
# Google remains the default among additive choices
...
is optimised for. The comparison below decides *what to lead with* while leaving the others available.
...
Passkeys are worth adding whenever a user wants stronger protection, with no need to drop Google
for everyone else.
...
* **Smaller security surface.** No password store exists to leak and no credential-stuffing
  target.
...
being the default. Losing default status calls for an ADR update. The ADR moves to **Deprecated**
only if the site removes Google sign-in entirely, and Better Auth (ADR 032)
