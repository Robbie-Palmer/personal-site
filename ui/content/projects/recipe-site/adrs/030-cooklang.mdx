...
so complex spice mixes can be named and reused across steps. There are 20+ community-built tools —
...
| **Cooking mode** | 🔧 Not provided — Cooklang has no polished kitchen UX. The iOS app is rated 3.8/5 with 4 reviews and crash reports; the CLI web server targets developers. Our large-text, step-by-step, wake-lock cooking mode remains entirely our responsibility and is our primary USP. |
...
3. **Ingredient aliasing not yet in spec** — [Discussion #41](https://github.com/cooklang/spec/discussions/41)
...
7. **Ecosystem maturity is mixed** — The spec and `cooklang-rs` are solid and actively maintained.
   However, the project is broad for an indie team and parts are spread thin: the iOS app has a
   3.8/5 rating with crash reports, some parsers in less common languages may be incomplete, and
   the federated index is nascent. We should treat the core Rust library and WASM bindings as
   production-ready; everything else warrants evaluation before depending on it.
...
- **Ecosystem maturity risk** — the core library (`cooklang-rs`, WASM bindings) is solid. The wider
  ecosystem (iOS app, some language parsers, federated index) is thinner. Evaluate each component
  individually before depending on it.
- **Spec evolution lag** — features in community discussion (ingredient aliasing) may take time to
