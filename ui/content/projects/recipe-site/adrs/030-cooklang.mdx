...
7. **Ecosystem maturity is mixed** — The spec and `cooklang-rs` are solid and actively maintained,
   which is what we actually depend on. The project is broad for an indie team and parts are spread
   thin: parsers in less common languages may be incomplete, the CookCLI is useful as a reference
   but warrants evaluation before hard-depending on specific features, and the federated index is
   nascent. Cooklang also ships a native iOS/Android app, but that is irrelevant to us — we
   wouldn't depend on it, and it actively contradicts our web-first/PWA rationale for
   cross-platform reach without app store overhead. Evaluate each open-source component
   individually; don't treat the brand as a monolith.
