...
> **Revision note (May 2026):** This ADR's original decision positioned `.cook` files as the
> permanent source of truth for recipe content. That stance has evolved.
> [ADR 033](/projects/recipe-site/adrs/033-application-data-store) moves recipe content to Neon
> Postgres, with Cooklang syntax preserved as a text column in the recipes table. `.cook` files
> remain the canonical *format* — parsed by the same `cooklang-rs` WASM bindings — but the
> database is now the canonical *storage location*. Existing `.cook` files serve as the import
> source for the initial recipe corpus. The Data Model Note section below has been updated to
> reflect this.
...
6. **No social or collaboration platform** — Cooklang is a file format, not a platform. User
...
8. **Federated search is emerging, not production** — The community recipe index is actively
   discussed but not a polished discovery mechanism yet.

## Data Model Note

Cooklang occupies a specific role in our data model — it is the **content format**, not the
storage layer:

* **Canonical format** — recipe content is authored and stored as Cooklang syntax (in a Postgres
  text column per [ADR 033](/projects/recipe-site/adrs/033-application-data-store))
...
* **Decision**: Rejected as the long-term content format. The instruction-text scaling gap is a
  ...
  a standard rather than joining one. There is no parser ecosystem to leverage, no community to
...
* **Decision**: Originally deferred. Now adopted per
  [ADR 033](/projects/recipe-site/adrs/033-application-data-store) — recipe content moves to Neon
  Postgres with Cooklang syntax preserved as a text column. The recipe site PRD anticipated this
  evolution: *"Move to a richer content workflow when the constraint actually bites."* UI-based
  recipe editing for non-technical household members is the moment the constraint bites.
...
* **Recipe editor UI is now load-bearing** — `.cook` syntax is not the blocker (it's simple), but
