...
first weekly archive of each month to a monthly prefix without exporting Neon a second time, and
retain monthly archives for 400 days.
...
required to produce a complete dump. Initially it is granted `CONNECT` and `pg_read_all_data`.
...
Read access is required only to copy the first weekly object into the monthly prefix. The R2 token
is scoped to the backup bucket and cannot administer unrelated R2 data.
...
cannot manage R2 lifecycle or bucket-lock rules, so configure those rules once through the R2
control plane:
...
CLI through the repository's mise configuration.

The job:

1. rejects a Neon URL whose hostname is a pooled endpoint
2. fails if `pg_dump`, encryption, checksum generation, upload, or size verification fails
3. opens one deduplicated GitHub issue when a run fails
4. supports manual dispatch for setup and recovery testing
