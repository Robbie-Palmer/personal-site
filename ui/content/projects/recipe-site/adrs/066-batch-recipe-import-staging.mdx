...
The coordinator applies explicit per-owner and per-provider active-job caps, reserves quota before
admission, and leaves excess work queued rather than starting unbounded workflows. Queue depth and
throttled state are visible to the workspace so backpressure is not presented as stalled work.
...
invalid entries receive item-level diagnostics while valid siblings can continue to review.
...
Prior confirmed versions are retained with the source artifacts for replay and audit.
