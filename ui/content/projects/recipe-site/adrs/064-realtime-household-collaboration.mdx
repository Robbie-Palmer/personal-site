...
state that later rolls back. Failure to broadcast after commit must:
...
outbox or adopt a change-data capture service in a separate decision.
...
30–60 seconds with no background polling when the page is hidden. WebSockets provide normal
low-latency updates; polling only bounds silent failure.
...
The mutation response remains canonical. On rejection, restore the captured cache state or refetch.
On success, reconcile the optimistic entity with the returned row and revision. TanStack Query
documents the corresponding
...
Every mutation carries a client-generated operation ID and is protected by a database uniqueness
constraint or mutation ledger. Repeating an accepted operation returns its recorded canonical
outcome rather than applying the change again. This provides retry-safe application semantics
without claiming transport-level exactly-once delivery.
...
5. **Client recovery:** add subscription readiness, initial snapshots, highest-revision tracking,
   gap detection, reconnect snapshots, visibility-aware repair polling, and stale-message rejection.
