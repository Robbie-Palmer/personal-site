...
Realtime delivery therefore reduces latency while Postgres remains the data-correctness boundary. A
...
A connection's authorization to receive household data expires.
...
Measure the exact active-page interval in the product instead of guaranteeing it in the protocol. Start around
...
Presence provides ephemeral coordination state. Never store it as durable business data.
...
The room identifier only routes messages. Knowing a household ID must never authorize
a subscription.
...
| **PartyKit over Durable Objects**                      | Reduces room, socket, reconnection, broadcast, local-development, and deployment boilerplate; offers a first-party Yjs integration and can deploy to the existing Cloudflare account without an additional platform fee. | PartyKit provides an implementation framework only. The application still owns Better Auth integration, household authorization, Neon commit ordering, conflict policy, recovery, and business validation. Its deployment/configuration must fit the existing Worker and preview topology.                   | Permit a thin spike before implementing the room. Adopt only if it materially reduces code and operations while preserving this ADR's trust, mutation, and recovery boundaries.                                       |
...
* The design tolerates reconnection but does not provide full offline-first behaviour.
