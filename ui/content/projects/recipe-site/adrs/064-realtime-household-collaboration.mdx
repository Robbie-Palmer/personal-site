...
low-latency updates; socket close/error, browser `online`, and window focus trigger immediate
reconnection and snapshot recovery. Polling only bounds otherwise silent failure and its interval
remains configurable from observed reliability.
...
Every mutation carries a client-generated operation ID. Persist it and its canonical outcome in
Neon in the same transaction that applies the mutation, protected by a unique
