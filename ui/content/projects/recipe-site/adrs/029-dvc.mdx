...
* I want to build the pipeline itself in TypeScript (tsx) so production and experimentation can
  reuse code. The tooling must not impose a Python-only
...
* **Cons**: The 100 experiment run limit on the free plan applies to private repositories. Since
  this repository is public, the limit should not block us, though this needs confirmation. Private
...
* **S3-compatible storage**: Works with any S3-compatible remote. Cloudflare R2 was chosen as the
  storage backend ([ADR 039](/projects/personal-knowledge-graph/adrs/039-cloudflare-r2)), satisfying the S3
...
* **Open source**: Apache 2.0 licensed. No proprietary lock-in. The community edition is identical to the
  paid offering. No feature-gated paywall exists.
