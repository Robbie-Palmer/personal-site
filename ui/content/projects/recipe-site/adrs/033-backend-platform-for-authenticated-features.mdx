...
has automatic global read replication across all 6 supported location hints at no additional cost —
an advantage Neon lacks. Writes go to the primary; reads are served from the nearest replica.
...
* **Single-region database.** Neon projects are locked to one of 8 AWS regions with no
  ...
  manual logical replication to a second Neon project, or revisiting D1 (which has automatic
