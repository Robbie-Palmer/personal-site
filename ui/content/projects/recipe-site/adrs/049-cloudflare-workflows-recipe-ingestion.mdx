...
The pipeline has multiple properties that make a normal request/response endpoint a poor fit:

* It is multi-step: upload, preprocess, extract, normalize, derive, canonicalize, validate, and
  publish or await review.
* It calls external providers multiple times, with different retry and timeout behavior per stage.
