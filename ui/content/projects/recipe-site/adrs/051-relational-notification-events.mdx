...
Database-level ways exist to enforce this stronger rule. A deferred constraint trigger could
...
This deliberately places the integrity boundary in the application. PostgreSQL could enforce cross-table
rules instead. If another service or manual writer starts creating notifications, reconsider deferred
constraint triggers so the database protects itself from every writer.
...
rows remain valid but no longer appear in anyone's archive. Here, "orphaned" means delivery-less
while referential integrity remains intact.
...
derive the available actions at read time. The diagram illustrates one possible runtime-authored model;
configurable behaviour does not require it.
