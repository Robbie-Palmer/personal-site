...
<Mermaid
  ...
  NOTIFICATION_EVENT ||--o| NOTIFICATION_HOUSEHOLD_INVITATION_EVENT : adds_invitation_context
  ORGANIZATION o|--o{ NOTIFICATION_HOUSEHOLD_EVENT : describes
  INVITATION o|--o{ NOTIFICATION_HOUSEHOLD_INVITATION_EVENT : references`}
/>

The subtype primary keys are also foreign keys to `notification_event`, enforcing at most one row
of each subtype per event. The unique `(event_id, recipient_user_id)` index prevents the same event
being delivered to one recipient twice.
...
The awkward rule is bidirectional: for example, a `household_invited` event must have exactly one
household subtype row and exactly one invitation subtype row, while a future recipe-comment event
must not have either. A foreign key enforces subtype-to-event existence, but not the reverse
event-to-required-subtype existence. PostgreSQL `CHECK` constraints only inspect the row being
