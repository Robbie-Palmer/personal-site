...
date: "2026-07-19"
status: "Accepted"
tech_stack: ["TanStack Query", "React", "Next.js"]
---
...
* may be read concurrently by multiple components;
...
| User recipe box, saved recipes, diet, profile   | multiple minutes + mutation invalidation | local writes are known; cross-device changes may arrive later     |
...
2. Define user-scoped query-key factories and shared retry/error classification before migrating
   components.
3. Migrate one vertical slice, for example recipe box, saved recipes, and diet, while retaining the neutral
