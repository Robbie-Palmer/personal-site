...
Mount one `QueryClientProvider` at the recipe application layout boundary, not the shared site
root. This keeps the dependency and cache out of unrelated site routes. Existing typed
...
details retain independent query keys. The client cache does not depend on aggregation, which remains
an optional API optimization.
...
mutation behavior add enough complexity to require a measured benefit.
