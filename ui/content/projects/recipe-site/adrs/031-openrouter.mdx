...
recouped via Vercel platform plan tiers). BYOK is available with a 5% fee on what the same
model would have cost on OpenRouter, after the first 1M BYOK requests per month free —
useful as a fallback if we ever want to use existing provider credits, but unnecessary for our
current scale. Spend stays predictable and pay-as-you-go, with no minimum commitment per
provider.
