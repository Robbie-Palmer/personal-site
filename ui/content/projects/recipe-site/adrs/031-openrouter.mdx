...
* **Pros**: Genuine OpenRouter-class product (catalogue + unified billing + retry). Pay-as-you-go
  with zero markup matches OpenRouter's economics. Native integration with the Vercel AI SDK if
  we ever adopt that client library.
...
platform fee on top of provider pass-through pricing. GDPR-native, EU data residency by
default, headquartered in France.

* **Decision**: **Rejected.** Compelling for projects that need multiple AI modalities under one
  contract (OCR + LLM + speech), but for our pure LLM/VLM use case the 5.5% markup makes
  OpenRouter cheaper, the LLM-specific routing controls are less granular, and the multimodal
  breadth beyond OCR is not on our roadmap. Worth a second look only if we wanted GDPR/EU data
  residency as a hard requirement.
