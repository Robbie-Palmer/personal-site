...
* **OpenAI-compatible REST API** — drop-in replacement for the OpenAI SDK by changing `baseURL`.
  Already in use at `ml-pipelines/recipe-parsing/src/lib/openrouter.ts:30-40`.
...
* **Pros**: Sits inline with the rest of the Cloudflare estate ([ADR 011](/projects/personal-site/adrs/011-cloudflare-pages),
  ...
  Free tier with strong observability (caching, analytics, logs). Recent Secrets Store
  integration ([Aug 2025](https://developers.cloudflare.com/changelog/post/2025-08-25-secrets-store-ai-gateway/))
  centralises API key storage.
* **Cons**: **Cloudflare AI Gateway is BYOK** — it does not provide its own catalogue or billing
  ...
  *replace* — every provider needs to be onboarded individually. There is also no first-party
  ...
  AI Gateway's strengths (logging, caching, rate limiting, budgets) are useful, but they sit
  *on top* of the per-provider sprawl rather than removing it.
* **Decision**: **Rejected as the primary access layer.** AI Gateway and OpenRouter solve
  ...
  any platform setup. We can always layer AI Gateway in front of OpenRouter later for caching
  and observability if usage scale justifies it; the SDK doesn't change.

## [Vercel AI SDK](https://sdk.vercel.ai/) + Provider Plugins

* **Pros**: Polished TypeScript ergonomics. Provider plugins exist for OpenAI, Anthropic, Google,
  Mistral, and many open-weight providers. Good streaming primitives and React hooks for any
  future chat UI.
* **Cons**: Doesn't eliminate the per-provider account/billing/key sprawl — it's a TypeScript
  facade over per-provider SDKs. Vercel also operates its own AI Gateway, which is BYOK and
  Vercel-platform-flavoured; we are not on Vercel ([Cloudflare Pages, ADR 011](/projects/personal-site/adrs/011-cloudflare-pages))
  and adopting it adds platform coupling. We can still consume the Vercel AI SDK with OpenRouter
  as the OpenAI-compatible provider if its DX wins are wanted later — the two are
  complementary, not exclusive.
* **Decision**: **Rejected as the access layer**, but kept open as a future client-side helper
  if a streaming chat UI is built. The transport remains OpenRouter.
