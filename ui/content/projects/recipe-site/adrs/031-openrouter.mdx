...
* **Pros**: Sits inline with the rest of the Cloudflare estate ([ADR 011](/projects/personal-site/adrs/011-cloudflare-pages),
  ...
  inspection on prompts and responses) and request data that stays within our Cloudflare
  account (relevant for regulatory data residency, not as a general observability win — see
  cons).
...
* **Decision**: **Rejected as the primary access layer.** Worth layering in front of OpenRouter
  *later* specifically if (a) guardrails become load-bearing for user-facing AI features, or
  (b) regulatory data residency requires request data to stay within our own infrastructure.
  Pure caching is not on its own reason enough — OpenRouter already covers it. General
  observability is *not* a reason to reach for AI Gateway; that's a separate decision about
...
* **Data flows through OpenRouter and the audit trail lives there, not on our infrastructure.**
  ...
  still transits OpenRouter's infrastructure. If regulatory data residency becomes a hard
  requirement (e.g. an enterprise customer needs all inference data to stay within
  infrastructure we control), Cloudflare AI Gateway in front (BYOK + Cloudflare Secrets
  Store + request data within our Cloudflare account) is the migration path without changing
  SDK code — purely as a data residency answer, not as an observability one.
...
* **Data residency becomes a hard requirement.** E.g. an enterprise customer or partner
  contract mandates that all inference request data stays within infrastructure we control
  (rather than transiting OpenRouter), or expanded operations require data to stay within a
  specific jurisdiction. Standard compliance frameworks (SOC 2, ISO 27001, GDPR) handle
  OpenRouter fine as a sub-processor with appropriate DPAs and vendor management — this
  bullet is specifically about residency, not generic compliance. Layering Cloudflare AI
  Gateway in front (BYOK + request data within our own account) or going BYOK direct for the
  affected providers is the response.
