...
Beyond ingestion, the [recipe-site roadmap](/projects/recipe-site) lists multiple
...
4. **Mixed-modality workloads.** Photo-to-recipe needs a vision-capable model; normalisation needs
   a cheap, fast text model; disambiguation can run on a small one. The same gateway must serve
   all three.
5. **Cooklang is the canonical output.** Whatever model is in the loop, the result lands in
   ...
   support across providers is load-bearing for the envelope, even though the
...
* **Cons**: Smaller catalogue (275 vs OpenRouter's 358 models, live counts May 2026) and
  ...
  Friendli) are almost entirely absent. For closed frontier models the depth is comparable (GPT-5.5:
  2 vs 2; DeepSeek V3.1: 6 vs 7), and Vercel   occasionally has more depth (Qwen 3 235B: 4 vs 1),
  but the open-weight long tail is where cost arbitrage lives and OpenRouter is decisively
...
Portkey self-serve at SMB scale, even though existing customers typically keep service on
