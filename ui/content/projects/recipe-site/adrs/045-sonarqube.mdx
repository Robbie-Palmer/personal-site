...
or expose the scanner logs needed to diagnose analysis warnings. The workflow generates coverage
before invoking the scanner, so the GitHub Actions log is the complete record of how an analysis
was assembled and uploaded.
...
Vitest reporter tuples pass their options to Istanbul's `lcov` reporter. Each workspace therefore
sets `projectRoot` so the generated `SF:` records are relative to the monorepo root and Sonar can
resolve them without a post-processing rewrite. This was verified against the installed Vitest and
Istanbul implementations and the generated reports after an automated reviewer incorrectly claimed
that the option was ignored.
