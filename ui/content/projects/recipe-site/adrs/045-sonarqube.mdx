...
| **CodeQL** ([ADR 024](/projects/recipe-site/adrs/024-codeql) → [Personal Knowledge Graph 032](/projects/personal-knowledge-graph/adrs/032-codeql)) | Security SAST         | Deterministic         | Deep dataflow for injection/XSS classes. Security only; not maintainability. |
| **Trivy** ([ADR 044](/projects/recipe-site/adrs/044-trivy) → [Personal Knowledge Graph 047](/projects/personal-knowledge-graph/adrs/047-trivy))    | IaC, deps, containers | Deterministic         | CVE/misconfig scanning. Not source quality.                                  |
...
[ADR 032](/projects/personal-knowledge-graph/adrs/032-codeql) rejected SonarQube for one specific reason: it
...
* Automatic Analysis is disabled. Analysis runs in CI via the official
  `SonarSource/sonarqube-scan-action`, pinned to a full commit SHA, on each PR and on `main`.
...
* The Sonar web project is used for occasional trend analysis, the role the GitHub Security tab
  already plays. Day-to-day work stays in GitHub.

# CI And Credentials

CI-based analysis is required because Automatic Analysis does not import external coverage reports
or expose the scanner logs needed to diagnose analysis warnings. An unrestricted job generates
coverage first, then a separate environment-bound job downloads those reports and invokes the
scanner. Fork pull requests therefore run the full test suite without waiting on protection rules
or receiving the upload credential.

The workflow caches Sonar's downloaded analyzer packages between GitHub-hosted runners and marks
`**/tests/**` as test code rather than production source. The latter keeps tests out of production
metrics and lets Sonar apply its test-specific rules. Sonar's server-backed sensor cache still owns
incremental code-analysis results; local CI caching is deliberately limited to the documented
package cache rather than persisting scanner working state.

`SONAR_TOKEN` is an authentication credential even though the repository and Sonar project are
public. Doppler remains its source of truth: the token is a masked value in `prd_ci_repo`, the sync
script publishes it as an environment secret in GitHub's `production-ci` environment, and only the
scanner step receives it. Trusted runs fail visibly when the token is absent. Fork pull requests
still run tests and generate coverage, but skip the upload because GitHub does not provide
environment secrets to forks.
...
[ADR 032](/projects/personal-knowledge-graph/adrs/032-codeql) declined to pay
...
Full analysis stays on. CodeQL remains the owner of the GitHub Security-tab SARIF workflow by
...
Sonar can resolve them without a post-processing rewrite. `lcovonly` avoids generating an unused
HTML coverage site, and coverage output directories are excluded from analysis defensively. This
was verified against the installed Vitest and Istanbul implementations and the generated reports
after an automated reviewer incorrectly claimed that the option was ignored.
...
| **Semgrep**                                   | Yes (SARIF)                  | Fast custom rules; named future SAST candidate in [ADR 032](/projects/personal-knowledge-graph/adrs/032-codeql). | A SAST/rules engine, not a code-health gate; overlaps CodeQL.                                | Rejected for this gap; remains the separate CodeQL-successor question.                                                          |
...
* Adds a deterministic, trend-tracked code-health signal as a GitHub PR check rather than a
  ...
  [ADR 032](/projects/personal-knowledge-graph/adrs/032-codeql).
