...
- **Cons**: The free plan restricts private repositories for commercial use — not a current blocker
  given the project is public and primarily a personal tool and portfolio piece, but a meaningful
  risk if it ever moves toward a commercial product, at which point the jump to the paid plan at
  ~$119/month is steep. Switching to DagsHub as the DVC remote would also mean migrating storage
  away from Cloudflare R2 and accepting DagsHub's storage pricing in its place.
- **Decision**: A strong candidate and genuinely well-suited to DVC-based CV workflows, but overkill
  for the current stage. The annotation and model management features become more compelling if the
  project scales up. Revisit if the evaluation workflow grows to need visual image review or a richer
  experiment UI.
