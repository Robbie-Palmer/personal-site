...
binary to track in git: even a modest initial batch of images at several megabytes each would bloat
every clone and make git operations slow, and the dataset is expected to grow to hundreds of images as
more of the physical recipe book is digitised. The dataset must be versioned — pinned to a specific
hash — so that every experiment can reproduce its exact inputs and results.
...
- **Cons**: Less widely adopted than DVC or MLflow — smaller community and fewer Stack Overflow
  ...
  on ClearML's servers rather than a purpose-built interactive UI. The all-in-one nature means
  migrating away later involves replacing more surface area.
...
- **Pros**: The most compelling DVC-native platform. Hosts DVC remote storage and an MLflow tracking
  ...
  developed with a growing ecosystem.
- **Cons**: The free plan does not permit private repositories for commercial use, which is a hard
  blocker for this project's private data. The paid plan that lifts this restriction starts at
  ~$119/month — disproportionate for a personal project. Storage would also be duplicated: DagsHub
  hosts its own DVC remote, which would sit alongside the existing Cloudflare R2 bucket rather
  than replacing it.
- **Decision**: Rejected on cost grounds. The platform is genuinely well-suited to DVC-based CV
  workflows and worth reconsidering if the project becomes commercial and the annotation/model
  management features become necessary. At personal-project scale, DVC directly against R2 achieves
  the same core workflow at zero additional cost.
...
- **No GUI for human review**: DVC has no built-in interface for visually browsing prediction
  outputs. `dvc exp show` renders a terminal table; `dvc plots diff` generates static HTML charts.
  For a computer vision problem — where reviewing model outputs means looking at images with
  overlaid annotations — the absence of a visual browser is a meaningful gap. If per-image
  inspection becomes a routine part of the evaluation workflow, a supplementary UI (W&B, DagsHub,
  or a bespoke viewer) would be needed.
