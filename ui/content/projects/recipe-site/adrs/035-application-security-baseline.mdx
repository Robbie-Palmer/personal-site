...
The Cloudflare Workers runtime provides several security properties at no cost:
...
These platform properties reduce the work left for the application-level controls above. The application
still needs explicit security engineering.
...
* **Cloudflare rate limiting rules** (configured via Terraform in
  `infra/public-platform/main.tf`) for broad endpoint protection, e.g. limiting requests per IP to
...
This **baseline ADR** leaves security-heavy product capabilities until evidence justifies
them.
