...
Multiple platform-level controls already exist elsewhere in the site architecture, including:
...
multiple security properties by default that reduce the baseline we need to implement ourselves.
...
The Cloudflare Workers runtime provides multiple security properties at no cost:
...
| **No user-managed server process** | No VM or container exists for us to patch, harden, or expose directly. Application state still belongs in explicit stores, not process memory.                                                 |
| **DDoS protection**                | Cloudflare's edge network provides baseline DDoS mitigation for all Workers. No configuration needed.                                                                                          |
| **No exposed ports**               | Workers are not addressable by IP. All traffic flows through Cloudflare's reverse proxy. No server exists to port-scan.                                                                        |
