...
which never learns that it owns the mission. Stable mission identities and the full invariant suite
come next. A consensus algorithm comes later, if an operation's evidence requirements justify one.
...
corruption, mission negotiation, retries, deterministic ties, busy nodes, health transitions,
32-bit clock rollover, explicit delivery faults, asymmetric links, and volatile state after a node
reset. A three-node in-memory simulation reconstructs the central demonstration without hardware.
