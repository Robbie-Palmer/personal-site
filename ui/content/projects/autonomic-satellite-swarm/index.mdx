...
mission requests have no expiry. Bounded telemetry comes next. A consensus algorithm comes later, if
an operation's evidence requirements justify one.
...
corruption, mission negotiation, stable keys, retries, deterministic ties, busy nodes, health
...
check against the native fixture. CI also compiles both firmware targets from pinned profiles.
