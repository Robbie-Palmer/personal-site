...
Here's a live interactive flowchart demonstrating Mermaid's capabilities for visualizing a software deployment pipeline:

<Mermaid chart={`graph TD
    A[Developer commits code] --> B{Run tests}
    B -->|Pass| C[Build application]
    B -->|Fail| D[Notify developer]
    D --> E[Fix issues]
    E --> A
    C --> F{Security scan}
    F -->|Vulnerabilities found| D
    F -->|Clean| G[Deploy to staging]
    G --> H{QA approval}
    H -->|Rejected| D
    H -->|Approved| I[Deploy to production]
    I --> J[Monitor metrics]
    J --> K{Performance OK?}
    K -->|Issues detected| L[Rollback]
    K -->|All good| M[Success!]
    L --> D
...
- **Multiple node shapes**: Rectangles for actions, diamonds for decision points
- **Directional flow**: Labeled edges show the process flow
- **Conditional branches**: Pass/fail paths and approval gates
- **Custom styling**: Individual nodes styled with colors
- **Complex workflows**: Feedback loops for error handling
- **Theme-aware**: The diagram automatically adapts to dark/light mode
