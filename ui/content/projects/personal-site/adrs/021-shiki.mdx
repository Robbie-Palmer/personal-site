...
Here are examples demonstrating Shiki's token-level accuracy across different languages:
...
These examples showcase Shiki's precision:
- **Python**: Decorator syntax (`@lru_cache`), type hints (`List[int]`), nested f-string expressions
- **SQL**: Window functions (`OVER`, `PARTITION BY`), CTEs, complex aggregations
- **TypeScript**: Generic constraints (`T extends`), async/await, nullish coalescing (`??`)
- All tokens are highlighted with VS Code-level accuracy, not just basic keyword matching
