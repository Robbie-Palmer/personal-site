...
* **Cons**: Exceptionally tedious. I would have to write custom CSS for every single HTML element (`h1` through `h6`, `p`, `ul`, `ol`, `li`, `blockquote`, `pre`, `code`, `table`, etc.) and handle all the spacing/margins between them. This is "undifferentiated heavy lifting."
...
* **Customizability**: I can still customize the theme via `tailwind.config.js` if I need specific brand overrides (like link colors or font families), but the defaults are strong.
