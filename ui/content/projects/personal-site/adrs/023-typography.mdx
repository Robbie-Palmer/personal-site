...
* **Why not**: Violates "Minimize Platforms, Maximize Velocity" and "Choose Boring Technologies" (in the sense of choosing the default path). It's a waste of time to reinvent typography styles.
...
* **Consistency**: Returns to the "Boring Technology" principle—using the tool designed by the framework authors for exactly this use case.
...
* **Customizability**: I can still customize the theme via `tailwind.config.js` if I need to specific brand overrides (like link colors or font families), but the defaults are excellent.
