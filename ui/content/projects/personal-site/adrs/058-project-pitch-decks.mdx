...
A small remark plugin will group root nodes between thematic breaks into slide nodes. Parsing the MDX syntax tree
avoids treating `---` inside code fences or JSX as a boundary. Inside `pitch.mdx`, a thematic break always
separates slides. Use another visual separator inside a slide.
...
`PitchNotes` will render as reveal.js speaker notes. The public Markdown converter will omit them. Notes
must not contain secrets. The HTML source is public even when the normal audience view hides the notes.
...
10. Unit tests cover deck discovery, frontmatter validation, slide grouping, transcript conversion, and tabs. A
    browser test covers navigation, focus-scoped keys, the focused route, and the mobile reading mode.
