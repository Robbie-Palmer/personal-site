...
* **Velocity**: Styling is considerably faster. I don't need to name classes or switch files.
* **Locality of Behavior**: Code and style live together. Deleting a component deletes its styles automatically.
* **AI Compatibility**: LLMs are strong at writing Tailwind. [LLM-Optimized](/projects?tab=philosophy#llm-optimized). The text-based nature of utility classes is easier for models to predict than arbitrary CSS class names + separate CSS files.
* **Consistency**: We are forced to use the design system values (spacing, colors) defined in the config, preventing "magic numbers".

### Cons

* **Markup Bloat**: HTML classes can become long and harder to scan (mitigated by component extraction and tooling).
