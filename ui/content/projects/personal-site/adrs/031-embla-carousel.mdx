...
Here's a complete example demonstrating Embla Carousel's headless API with custom navigation:
...
function ImageCarousel() {
  ...
  const images = [
    { id: 1, title: 'Mountain Sunset', url: '/images/mountain.jpg' },
    { id: 2, title: 'Ocean Waves', url: '/images/ocean.jpg' },
    { id: 3, title: 'Forest Path', url: '/images/forest.jpg' },
    { id: 4, title: 'Desert Dunes', url: '/images/desert.jpg' },
  ];
    ...
    <div className="relative">
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="flex gap-4">
          {images.map((image) => (
            <div key={image.id} className="flex-[0_0_100%] md:flex-[0_0_50%] lg:flex-[0_0_33.33%]">
              <img
                src={image.url}
                alt={image.title}
                className="w-full h-64 object-cover rounded-lg"
              />
              <h3 className="mt-2 text-center font-medium">{image.title}</h3>
            </div>
          ))}
        </div>
      </div>
...
- **Headless architecture**: No pre-styled components—navigation buttons use custom Tailwind classes
- **Hook-based API**: `useEmblaCarousel` returns refs and API for carousel control
- **Plugin integration**: `AutoScroll` plugin enables automatic scrolling without bloating the core
- **Responsive slides**: CSS classes control slides per view (1 on mobile, 2 on tablet, 3 on desktop)
- **Full control**: Custom navigation buttons trigger `scrollPrev`/`scrollNext` programmatically
- **TypeScript-ready**: Fully typed API with `EmblaCarouselType` inference
