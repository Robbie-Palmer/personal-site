...
I need a carousel component to display blog posts on the landing page with auto-scrolling and responsive behavior (1-3 posts per view across mobile to desktop).

Traditional carousel solutions present a few tradeoffs:
...
* **Lightweight**: The core is \~6KB gzipped, considerably smaller than Swiper (\~31-45KB). We only load the auto-scroll plugin (\~2KB) when needed.
...
* **TypeScript-First**: Strong type inference with `EmblaCarouselType` and `EmblaOptionsType`. The API is fully typed, reducing runtime errors and improving the agentic workflow.
