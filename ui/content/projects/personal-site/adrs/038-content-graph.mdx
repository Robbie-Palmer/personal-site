...
The fixed Sigma layout made filtering predictable, but it also made the graph feel static. Rebuilding the Graphology instance after every filter change discarded the previous layout, synchronous ForceAtlas2 work blocked the main thread, and rendered node radii could still overlap.

Replace Sigma.js and Graphology with **[cosmos.gl](https://github.com/cosmosgl/graph)**. Keep every node at a stable numeric index and hide filtered nodes with absent positions. The GPU simulation can then reheat the remaining graph without rebuilding its identity.

The replacement adds:

* Radius-aware collision and a continuously settling force layout
* Animated filtering while preserving the positions of surviving nodes
