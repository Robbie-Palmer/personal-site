...
Replace Sigma.js and Graphology with **[cosmos.gl](https://github.com/cosmosgl/graph)**. Keep every node at a stable numeric index and hide filtered nodes with absent positions. A deterministic radial layout keeps the initial graph inside its fitted viewport without rebuilding its identity. Short, low-energy force simulations run only after dragging or filtering.

The replacement adds:

* A stable initial layout that remains inside the fitted viewport
* Contained physics after dragging or filtering, while preserving surviving node positions
