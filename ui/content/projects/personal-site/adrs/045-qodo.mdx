...
1. **CodeRabbit** now assigns this repository the bottom of its variable open-source allowance: one review per hour. Each reviewed push consumes that allowance, so a Pull Request developed through a few commits can be delayed repeatedly rather than receiving one allowance for the Pull Request as a whole.
