...
* **Remix**: Focuses on web standards and progressive enhancement. Strong data loading patterns, but slightly smaller ecosystem than Next.js.
* **Astro**: Good for content-heavy sites (like this portfolio). Uses "Island Architecture" to ship zero JS by default. However, complex interactive state management between islands can be trickier than a unified React tree.
* **TanStack Start**: A specific framework wrapper around TanStack Router. Promising, but currently too "bleeding edge" for a foundational infrastructure project.
