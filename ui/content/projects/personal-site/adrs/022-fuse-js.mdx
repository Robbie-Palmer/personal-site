...
* **Algolia**: Hosted search-as-a-service with strong UX, but expensive (pricing escalates with usage) and requires external platform dependency
...
* **MiniSearch**: Small and lightweight, but less feature-rich and smaller community
...
* **Larger Bundle**: \~25KB gzipped is larger than FlexSearch (\~7KB). However, this is acceptable for a content-focused site where search considerably improves UX.
* **Client-Side Performance**: Search runs on the user's device. For large datasets (thousands of posts), this could be slow on low-end devices. Currently fine for a personal blog with fewer than 100 posts.
