...
This aligns with [Minimize Platforms](/projects?tab=philosophy#minimize-platforms) and [LLM-Optimized](/projects?tab=philosophy#llm-optimized). Fuse.js is self-hosted (no external services), has extensive LLM training data, and works seamlessly with static site generation.
...
* **Platform Independence**: Aligns with [Minimize Platforms](/projects?tab=philosophy#minimize-platforms). No external dependencies on Algolia, Pinecone, Elasticsearch clusters, or vector databases. Everything runs in the browser.
