...
Results are uploaded to GitHub Security tab in SARIF format for unified visibility alongside CodeQL findings. PR gating uses GitHub's Code Scanning alert checks (the same mechanism as CodeQL ([ADR 032](/projects/personal-site/adrs/032-codeql))), which distinguish new alerts from pre-existing ones. Only vulnerabilities introduced by a PR block it.
