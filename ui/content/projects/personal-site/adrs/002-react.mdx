...
My vision for this platform extends beyond a simple static blog; it is intended to be an incubator for future SaaS projects, a testing ground for new technologies, and a place to demonstrate engineering thinking (see [Context](/projects/personal-site/index)).
...
* **Static Site Generators (Jekyll, Hugo)**: Good for text-heavy content, but adding interactivity requires "sprinkling" jQuery or Vanilla JS, leading to a fragmented codebase ("spaghetti code")
* **Modern Component Frameworks (Vue, Svelte, Angular, Solid)**:
  ...
  * **Svelte/Vue**: Strong ergonomics and often smaller bundle sizes. However, they rely on custom template syntax (`v-if`, `{#if}`) rather than JavaScript control flow
...
This aligns with [The Goldilocks Zone](/projects?tab=philosophy#the-goldilocks-zone). React is the web's "standard library" for UI.
...
* **vs Svelte/Vue**: While Svelte/Vue have passionate communities and often superior ergonomics for small projects, React's ecosystem dominance is the deciding factor. The probability that a library I need exists for React is considerably higher than for any other framework
* **vs Angular**: React's "Just JavaScript" philosophy aligns better with my preference for composable primitives over rigid framework conventions
...
* **AI Velocity**: Because React is the most popular framework, it is the highest-represented language in LLM training sets. Models like Claude and GPT are exceptionally good at writing React, [LLM-Optimized](/projects?tab=philosophy#llm-optimized) by default
