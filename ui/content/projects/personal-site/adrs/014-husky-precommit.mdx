...
* **Integrates with Mise**: Build on top of the task system established in [ADR 003: Mise](/projects/personal-site/adrs/003-mise), not duplicate configuration
...
This builds directly on top of mise tasks from ADR 003, avoiding duplication. The tasks in `.mise.toml` accept file arguments, allowing lint-staged to pass only staged files rather than running against the entire codebase.
...
* **Cons**: Less mature ecosystem, fewer npm downloads (\~500K/week vs Husky's 8M/week), less LLM training data for AI-assisted development
...
* **Claude Code Guardrails**: Prevents [Claude Code](/projects/personal-site/adrs/011-claude-code) from drifting too far and committing malformed code. Pre-commit hooks act as an immediate correction signal, keeping the AI agent aligned with project standards without requiring constant human oversight.
