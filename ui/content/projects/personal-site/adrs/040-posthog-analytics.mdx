...
* **Cons**: Intentionally minimal—covers only tier 1 (basic analytics). No custom events beyond simple goals. No user flow analysis, no A/B testing, no session replay, no feature flags. Would require adding a second platform as soon as needs go beyond page views. Paid only (no free tier for cloud-hosted).
...
* **Cons**: Focused on product analytics rather than web analytics—less suited for content-heavy sites. No built-in A/B testing, feature flags, or session replay. Would still require additional platforms for experimentation. More complex setup for what starts as a simple analytics need.
...
* **Cons**: Similar to Mixpanel in scope—strong on product analytics, weak on the broader platform features (no feature flags, no A/B testing, no session replay). Enterprise-oriented UX that is heavyweight for a personal project.
