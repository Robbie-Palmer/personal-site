...
Cloudflare provides basic analytics in its dashboard (page views, unique visitors, top paths), but the data is severely limited and the UX for exploring it is poor. It offers no way to track user flows, define success metrics, run experiments, or build a meaningful feedback loop from Cloudflare's built-in analytics alone.

The requirements break down into a few tiers:
...
* **Cons**: Limited data granularity. No custom events, no user flows, no funnels, no A/B testing. The dashboard UX makes it difficult to extract actionable insights even from the basic data it does collect. No path to the deeper analytics tiers without adding another platform.
...
* **Cons**: Privacy concerns. Google uses analytics data for its own advertising purposes. Requires cookie consent banners for GDPR compliance. Complex configuration for custom events and funnels. The GA4 migration fragmented the ecosystem and degraded UX considerably. Does not offer feature flags, A/B testing, or session replay, would still need additional platforms for those capabilities.
