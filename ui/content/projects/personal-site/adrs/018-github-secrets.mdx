...
* **Developer-focused Managers ([Doppler](https://www.doppler.com/))**: Strong DX and specialized for this problem (and has a free tier). However, it introduces "Another Platform" to manage. It also requires a "Secret Zero" (`DOPPLER_TOKEN`) to be injected into the CI environment to allow access
...
* **Security**: Support for OIDC allows for "keyless" authentication, considerably reducing the attack surface. If a token leaks, it expires automatically when the job ends
* **Solves "Secret Zero"**: By being integrated into the platform, GitHub *is* the identity provider. No bootstrap token is needed
