...
Knowledge work changes what we know, including whether a planned task still
...
queue, then pull its highest-priority eligible task when capacity becomes
...
limited to the delivery of an individual task once picked up. Queued work does
...
policy, and attach the evidence needed to close a task. The graph must also work
...
3. Expose a narrow agent interface for reading ready work and proposing graph
   changes, including the assumptions and evidence behind a revision.
...
8. Run real projects through it before adding the Agent Coordinator.
   Test whether discoveries can replace a provisional branch while unrelated
