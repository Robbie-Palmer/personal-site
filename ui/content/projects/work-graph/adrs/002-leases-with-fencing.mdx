...
Opening a blocking attention request ends the active lease. Resolving every
blocking request makes the item ready for any worker. Preserve the previous
worker in lease history so a later queue policy can give continuity a soft
preference without reserving the item.
