...
### Cloudflare Pages: Right-Sized for Extensibility

**Why Cloudflare over Vercel?**

Cloudflare provides the **right balance** for current needs and future ambitions:

- **DDoS protection**: Built-in, not an add-on (essential for public sites)
- **Edge compute**: Workers at 300+ locations globally
- **Extensibility without AWS weight**: Can add databases (D1), KV storage, R2 object storage without introducing a heavyweight CSP like AWS
- **Pricing transparency**: Generous free tier, predictable pricing

Vercel is excellent for Next.js, but lacks the infrastructure primitives I'll need for future server-side features.

AWS would be overkill—too much operational complexity for a personal site.
...
- **Edge functions**: Available when I need serverless compute

### Terraform: Right-Sizing Operational Complexity
...
**Terraform Cloud for state, GitHub Actions for execution:**

I use **Terraform Cloud** for state storage, but run `terraform plan` and `terraform apply` in **GitHub Actions**:
...
**Why this split setup?**

- **Terraform Cloud for state** = Less operational complexity than managing S3 buckets, DynamoDB locks, or local state
- **GitHub Actions for execution** = I regularly review `terraform plan` outputs on PRs (need them visible, not buried in Terraform Cloud UI)
- **Secrets in GitHub Environments** = Single source of truth for credentials

This mirrors my ML workflow: results get published to PRs for review, not hidden in separate dashboards.

**Why Terraform over OpenTofu?**

Pragmatism. I prefer open tooling theoretically, but Terraform Cloud's state management is **operationally simpler** than:
- Running my own state backend (S3 + DynamoDB)
- Using another CSP just for state storage
- Managing OpenTofu + separate state solution

The goal isn't minimizing complexity—it's **right-sizing operational complexity** for current needs and future ambitions.
...
4. **Right-size operational complexity**: Use the best tool for each job. Terraform Cloud for state (simpler than S3/DynamoDB), GitHub Actions for execution (need plan outputs visible on PRs), Cloudflare for hosting (DDoS + edge compute without AWS weight). The goal isn't minimizing services—it's minimizing operational burden.
...
8. **PR-centric workflow pattern**: Both terraform plans and ML experiment results get published to PRs for review. Keep critical information visible where decisions are made, not hidden in separate dashboards or UIs.
...
- **Right-sized operational complexity** → Best tool for each job (Terraform Cloud for state, GitHub Actions for execution, Cloudflare for hosting)
...
- **PR-centric workflow** → Terraform plans and results published to PRs for review (like ML workflows)
