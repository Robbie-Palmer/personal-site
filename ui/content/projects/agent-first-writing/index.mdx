...
Version 1 of the record contract now lives in the shared
`writing-editor-domain` package. It hashes exact source bytes, uses half-open
UTF-8 spans, distinguishes findings without safe rewrites from actionable
suggestions, groups suggestions under stable proposal IDs, and keeps the full
proposal beside each accepted, rejected, or changed decision.

The evaluation pipeline now extracts nine public before-and-after pairs from
pinned Git commits and freezes a 5/2/2 train, validation, and holdout cohort.
Its readiness check still fails by design. Git shows the published edit, but it
does not reveal whether the author accepted, rejected, or changed an editor
proposal. Recorded outcomes and the GECToR baseline must enter private DVC
storage before scoring can start.

The first producer stage now runs the pinned Vale binary against all nine
frozen source artifacts. It emits 43 stable findings across the cohort, with
rule provenance and source-verified UTF-8 spans. These findings mark passages
for the next rewrite producer. They do not claim that Vale knows a safe
replacement.

The next pipeline stage aligns those findings with the published revisions. It
found 15 self-contained changes and sent 28 boundary-crossing edits to manual
adjudication. Each result retains the exact before-and-after bytes and stable
edit-hunk IDs. This measures where published edits touched a Vale finding
without pretending that the diff records an accepted suggestion.
