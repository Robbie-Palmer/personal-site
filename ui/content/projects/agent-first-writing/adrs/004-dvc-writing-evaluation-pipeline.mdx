---
title: "ADR 004: DVC writing evaluation pipeline"
date: "2026-09-06"
status: "Proposed"
tech_stack: ["DVC"]
---
...
Run each DVC stage through a checked-in shell script or direct command. Hash the
script, source, lockfile, parameters, and data inputs that determine its output.
Do not make a stage depend on the root `mise.toml`: unrelated development-tool
changes should not invalidate cached evaluation artifacts. A stage that uses a
mise-installed binary must validate its declared version at runtime.

This proposal uses the accepted record contract from
[ADR 003](/projects/agent-first-writing/adrs/003-versioned-editor-record-contract).
Producer stages validate their findings, suggestions, proposals, and decisions
against that shared contract before writing DVC outputs.
...
2. `freeze_cohort` fixes the train, validation, and holdout membership before
...
3. `run_producers` runs Vale, the GECToR-2024 baseline, and each declared
   candidate against the frozen inputs. Detection-only rules write versioned
   findings. Producers with a safe replacement write suggestions and proposals.
   The shared domain package validates both formats.
   Captured Grammarly suggestions enter as reference data, not through an
   automated Grammarly stage.
4. `match_edits` first aligns stable finding IDs with the final published diff.
   As rewrite producers arrive, it will also align proposal IDs and their
   ordered member suggestion IDs with recorded accepted, rejected, and changed
   decisions. It keeps observed changes, explicit outcomes, unmatched records,
   and ambiguous cases separate.
