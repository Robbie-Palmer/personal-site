...
image: "blog/the-philosophy-of-data-science-featured-2025-12-14"
imageAlt: "Thick colourful threads forming a human head. An artist's illustration of AI and the complexity of biology."
---
...
An Empiricist cannot reproduce every experiment themselves,
...
Yet these rules of correspondence are themselves theories, which need to be supported.
...
Yet it does not view what is unscientific as meaningless.
...
Popper found value in the unscientific while criticising the pseudo-scientific.
...
neither does "theoretical fit". Evolutionary steps can stray from objective truth. Conjecture and refutation
instead drive us towards more interesting problems.
...
Kuhn's theory of paradigm shifts says evidence can occasionally force whole worldviews to change,
carrying their individual theories with them. (Bird, 2018)
...
Both Positivists and Critical Rationalists believe that scientific revolutions and "normal science"
...
What even is the target? Different targets have no common measure, even when abstractly targeting the same goal.
...
Kuhn claims "revisionary change" is a necessary condition for a scientific revolution.
Aka a paradigm must be overthrown as in the case of Einstein's impact.

Some critics point to what they view as scientific revolutions occurring without paradigm shifts,
...
difference in semantics applied to the word "revolution".
...
Yet they do not claim that this is tractable / accessible, like the Logical Positivists did.
...
An iterative process of completing a theory produces the model.
...
Yet many systems elude simplification via mathematical formulae.
...
exists, but it cannot escape the need for heavy computation. Greater computation in this case provides greater precision.
...
Rather, scientific progression has been guided by which theories happen to be
...
A scientific revolution requires a full shift to a new paradigm.
...
Infinitely many problem framings exist, so it is impossible to derive an appropriate context from an arbitrary datum;
...
These derive from how people hold the philosophy in practice rather than its fundamental truth claims.
