...
PR previews remain behind Cloudflare Access. Agent runtimes receive the
preview-only `CF_ACCESS_CLIENT_ID` and `CF_ACCESS_CLIENT_SECRET` credentials
from their secret store. For HTTP inspection, use the allowlisted wrapper so
the credentials can only be sent to a canonical `pr-<number>` Pages hostname:
...
`CF-Access-Client-Id` and `CF-Access-Client-Secret` extra HTTP headers before
navigating to the preview. Scope the headers to the canonical preview hostname;
never type, log, or commit either credential.
